...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`commitments_locales\` RENAME COLUMN "call_to_action" TO "link_text";`)
  await db.run(sql`ALTER TABLE \`commitments_locales\` ADD \`content\` text;`)
  ...
  await db.run(sql`ALTER TABLE \`commitments\` ADD \`info_article_id\` integer REFERENCES articles(id);`)
  await db.run(sql`CREATE INDEX \`commitments_info_article_idx\` ON \`commitments\` (\`info_article_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`commitments_locales\` RENAME COLUMN "link_text" TO "call_to_action";`)
  await db.run(sql`ALTER TABLE \`commitments_locales\` DROP COLUMN \`content\`;`)
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_commitments\`("id", "emoji", "impact_score", "slug", "parent_id", "updated_at", "created_at") SELECT "id", "emoji", "impact_score", "slug", "parent_id", "updated_at", "created_at" FROM \`commitments\`;`)
