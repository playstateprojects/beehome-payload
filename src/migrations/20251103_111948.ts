...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Check if in_app_notifications_valid_months table exists
  const inAppValidMonthsTable = await db.run(sql`SELECT name FROM sqlite_master WHERE type='table' AND name='in_app_notifications_valid_months';`)
  if (!inAppValidMonthsTable.results || inAppValidMonthsTable.results.length === 0) {
  ...
  // Check if in_app_notifications_texts table exists
  const inAppTextsTable = await db.run(sql`SELECT name FROM sqlite_master WHERE type='table' AND name='in_app_notifications_texts';`)
  if (!inAppTextsTable.results || inAppTextsTable.results.length === 0) {
  ...
  // Check if push_notifications_valid_months table exists
  const pushValidMonthsTable = await db.run(sql`SELECT name FROM sqlite_master WHERE type='table' AND name='push_notifications_valid_months';`)
  if (!pushValidMonthsTable.results || pushValidMonthsTable.results.length === 0) {
  ...
  // Check if push_notifications_texts table exists
  const pushTextsTable = await db.run(sql`SELECT name FROM sqlite_master WHERE type='table' AND name='push_notifications_texts';`)
  if (!pushTextsTable.results || pushTextsTable.results.length === 0) {
  ...
  // Check if condition_notes column exists in in_app_notifications
  const inAppTableInfo = await db.run(sql`PRAGMA table_info(in_app_notifications);`)
  const inAppColumns = inAppTableInfo.results as Array<{ name: string }>
  const conditionNotesExists = inAppColumns.some(col => col.name === 'condition_notes')

  if (!conditionNotesExists) {
