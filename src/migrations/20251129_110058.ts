...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`articles_rels\` ADD \`space_types_id\` integer REFERENCES space_types(id);`)
  await db.run(sql`CREATE INDEX \`articles_rels_space_types_id_idx\` ON \`articles_rels\` (\`space_types_id\`);`)
  await db.run(sql`ALTER TABLE \`_articles_v_rels\` ADD \`space_types_id\` integer REFERENCES space_types(id);`)
  await db.run(sql`CREATE INDEX \`_articles_v_rels_space_types_id_idx\` ON \`_articles_v_rels\` (\`space_types_id\`);`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  await db.run(sql`CREATE TABLE \`__new_articles_rels\` (
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_articles_rels\`("id", "order", "parent_id", "path", "commitments_id", "articles_id", "bee_info_id", "questionnaire_id") SELECT "id", "order", "parent_id", "path", "commitments_id", "articles_id", "bee_info_id", "questionnaire_id" FROM \`articles_rels\`;`)
  await db.run(sql`DROP TABLE \`articles_rels\`;`)
  await db.run(sql`ALTER TABLE \`__new_articles_rels\` RENAME TO \`articles_rels\`;`)
  ...
  await db.run(sql`CREATE TABLE \`__new__articles_v_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  ...
  `)
  await db.run(sql`INSERT INTO \`__new__articles_v_rels\`("id", "order", "parent_id", "path", "commitments_id", "articles_id", "bee_info_id", "questionnaire_id") SELECT "id", "order", "parent_id", "path", "commitments_id", "articles_id", "bee_info_id", "questionnaire_id" FROM \`_articles_v_rels\`;`)
  await db.run(sql`DROP TABLE \`_articles_v_rels\`;`)
  await db.run(sql`ALTER TABLE \`__new__articles_v_rels\` RENAME TO \`_articles_v_rels\`;`)
