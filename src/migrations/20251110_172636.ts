...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Check if action_cards_valid_months table exists
  const validMonthsTable = await db.run(sql`SELECT name FROM sqlite_master WHERE type='table' AND name='action_cards_valid_months';`)
  if (!validMonthsTable.results || validMonthsTable.results.length === 0) {
  ...
  // Check if columns exist in action_cards
  const actionCardsInfo = await db.run(sql`PRAGMA table_info(action_cards);`)
  const actionCardsColumns = actionCardsInfo.results as Array<{ name: string }>

  const scheduleExists = actionCardsColumns.some(col => col.name === 'schedule')
  const limmitExists = actionCardsColumns.some(col => col.name === 'limmit')
  const multipleViewsExists = actionCardsColumns.some(col => col.name === 'multiple_views')
